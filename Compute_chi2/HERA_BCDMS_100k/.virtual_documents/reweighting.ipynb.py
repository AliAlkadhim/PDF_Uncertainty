...
MVN_100k = np.load('../MVN_samples/MVN_100k.npy'); MVN_100k


MVN_25k = np.load('MVN_samples/MVN_25k.npy'); MVN_25k


chi2_100k=np.load('collected_chi2.npy'); chi2_100k
...
axes[0,0].hist(MVN_100k[:,0],bins=100, label='Bg')
axes[0,1].hist(MVN_100k[:,1],bins=100, label='Cg')
axes[0,2].hist(MVN_100k[:,2],bins=100,label='Aprig')
axes[0,3].hist(MVN_100k[:,3],bins=100, label='Buv')
axes[1,0].hist(MVN_100k[:,4],bins=100, label='Cuv')
axes[1,1].hist(MVN_100k[:,5],bins=100,label='Euv')
axes[1,2].hist(MVN_100k[:,6],bins=100, label='Bdv')
axes[1,3].hist(MVN_100k[:,7],bins=100, label='Cdv')
axes[2,0].hist(MVN_100k[:,8],bins=100, label='CUbar')
axes[2,1].hist(MVN_100k[:,9],bins=100,label='DUbar')
axes[2,2].hist(MVN_100k[:,10],bins=100,label='ADbar')
axes[2,3].hist(MVN_100k[:,11],bins=100,label='BDbar')
axes[3,0].hist(MVN_100k[:,12],bins=100,label='CDbar')
axes[3,1].hist(MVN_100k[:,13],bins=100,label='CDbar')
axes[3,2].hist(MVN_100k[:,13],bins=100,label='CDbar')
axes[3,3].hist(MVN_100k[:,13],bins=100,label='CDbar')
...
#plt.savefig('HERAPDF_params_MVN_100k_unweughted.png', dpi=300, bbox_inches='tight')
plt.show()


MVN_100k_chi2 = np.load('collected_chi2.npy')
dof = 377
MVN_100k_chi2_per_dof=MVN_100k_chi2/377
MVN_100k_chi2


mean_chi2 = np.mean(MVN_100k_chi2)
chi2_diff = MVN_100k_chi2 - mean_chi2
...
weights_Bg = 26481*weights_Bg/np.sum(weights_Bg)
...
plt.hist(Bg.flatten(), range=(-0.2,-0.003),bins=100, alpha=0.35, label=r'$B_g$ Unweighted Distribution')
n, bins, patches=plt.hist(Bg.flatten(), weights=weights_Bg, color='r',range=(-0.2,-0.003),bins=100, alpha=0.35, label=r'$B_g$ Weighted Distribution')
...
weights_Ag = 26481*weights_Ag/np.sum(weights_Ag)
...
Bg = MVN_100k[:26481,0]; weights_Bg


params=[]
for i in range(14):
    params.append(np.array(MVN_100k[:26481,i]))
params[0]


# weights = np.empty((26481, 2))
...
weights = np.empty((26481, 14))
for i in range(13):
    weights[:,i] = np.exp(-0.5 * (chi2_diff))/MVN_100k[:26481,i]
    weights[:,i] = 26481 * weights[:,i]/np.sum(weights[:,i])
print(weights.shape, '\n\n', weights[:,0], '\n\n', weights[:,0].mean(), '\n\n', weights[:,0].std())
...
# MVN1=MVN_100k[:26481,i].flatten()-0.05
# MVN2=MVN_100k[:26481,i].flatten()+0.02
# import matplotlib.pyplot as plt
# import matplotlib.pyplot as plt
...
# for i in range(14):
#     axes[i,0].hist(MVN_100k[:26481,i].flatten(), bins=100)
...
#PLOT UNWEIGHTED DISTRIBUTIONS (AT COL 0)
for i in range(14):
    axes[i,0].hist(MVN_100k[:26481,i].flatten(), bins=50)
...
#PLOT WEIGHTED DISTRIBUTIONS
for i in range(14):
    axes[i,1].hist(MVN_100k[:26481,i].flatten(), weights=weights[:,i], bins=50, color = 'r')
    #axes[i,1].set(title=titles[i] + ' Weighted', xlabel='value')
    axes[i,1].set_title(titles[i] + ' Weighted',size=25)
    axes[i,1].set_xlabel('value', size=20)
...
# uw_hist =  np.histogram(MVN_100k[:26481,0], bins=50)
# plt.hist(uw_hist[1].flatten(), bins=50)


unweighted_hists = []
for i in range(13):
    uw_hist, uwbins =  np.histogram(MVN[:26481,i])
...
#PLOT UNWEIGHTED DISTRIBUTIONS (AT COL 0)
for i in range(14):
    axes[i,0].hist(MVN_100k[:26481,i], bins=50)
...
for i in range(14):
    ...
    axes[i,1].hist(MVN_100k[1:,i][filter_mask[:,i]], weights=weights[:,i][filter_mask[:,i]], bins=50, color = 'r')

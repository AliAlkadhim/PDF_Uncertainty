...
MVN_4000 = np.load('/home/ali/Desktop/Pulled_Github_Repositories/NNPDF_Uncertainty/master_version/local/ALL_DATA_25k/chi2_array_ALL_DATA_25k.npy'); MVN_4000
...
# chi2_array_ALL_DATA_4k = chi2_array_ALL_DATA_4k.astype(np.float128)
# MVN_4000_MASTER = MVN_4000_MASTER.astype(np.float128)
plt.hist(chi2_array_ALL_DATA_4k, bins=100, range=(4000,70000))
plt.title('$\chi^2$ for All data', fontsize=20)
chi2_array_ALL_DATA_4k
...
plt.hist(weight_2, bins=50, range=(0,10))
...
MVN_4000_MASTER.shape


MVN_4000_MASTER[1:,],  MVN_4000_MASTER[2:,], MVN_4000_MASTER.shape
...
log_RHS = np.empty((25000,14))
for i in range(14):
    log_RHS[:,i] = log_numerator[:,i] - log_den[:,i]
    print(np.mean(log_RHS[:,i]))
# print('log(RHS) = ', log_RHS[:,0], '\n')
# print('therefore, log(w) = log(RHS)')



mean_log_RHS = np.empty((25000,14))
for i in range(14):
    mean_log_RHS[:,i] = np.mean(log_RHS[:,i])

weights = np.empty((25000,14))
for i in range(14):
    weights[:,i] = log_RHS[:,i] - mean_log_RHS[:,i]
weights[:,0]


plt.hist(weights[:,0], alpha=0.3)
plt.hist(weights[:,1], alpha=0.3)


for i in range(14):
    weights[:,i] = 25000 * weights[:,i]/np.sum(weights[:,i])
weights[:,i]


titles = ['$B_g$','$C_g$','$A_g$','$B_g$','$B_{u_v}$','$C_{u_v}$','$E_{u_v}$','$B_{d_v}$','$C_{d_v}$','$C_{Ubar}$','$D_U$','$A_{Dbar}$','$B_{Dbar}$','$C_{Dbar}$']
for i in range(14):
    plt.hist(weights[:,i], label='weights for '+titles[i] + 'calculated according to eq. (1)', bins=100, alpha=0.3, range=(0,10))

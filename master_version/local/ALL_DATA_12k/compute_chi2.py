...
import numpy as np
import re
MVN_12k_MASTER = np.load('/home/ali/Desktop/Pulled_Github_Repositories/NNPDF_Uncertainty/master_version/samples/MVN_12k_MASTER.npy')
...
for sample_ind in range(num_samples):
    ...
    with open(minuit_in_path, 'w') as second:
        ...
        second.write('  Adbar   : [ ' + str(format(MVN_12k_MASTER[:,0][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Agp   : [ ' + str(format(MVN_12k_MASTER[:,1][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Bdbar   : [ ' + str(format(MVN_12k_MASTER[:,2][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Bdv   : [ ' + str(format(MVN_12k_MASTER[:,3][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Cgp   : [ ' + str(25.000) + ', 0. ]\n')
        #note that Cprig is a constant, not a parameter value!
        second.write('  Auv  :  DEPENDENT\n')
        second.write('  Bg   : [ ' + str(format(MVN_12k_MASTER[:,4][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Bgp   : [ ' + str(format(MVN_12k_MASTER[:,5][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Duv  : [    0     ]\n')
        second.write('  Buv   : [ ' + str(format(MVN_12k_MASTER[:,6][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Adv  :  DEPENDENT\n')
        second.write('  Cdbar   : [ ' + str(format(MVN_12k_MASTER[:,7][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Cdv   : [ ' + str(format(MVN_12k_MASTER[:,8][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Aubar: [ 0.0, 0.0 ]\n')
        second.write('  Bubar: [ 0.0, 0.0  ]\n')
        second.write('  Cg   : [ ' + str(format(MVN_12k_MASTER[:,9][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Cubar   : [ ' + str(format(MVN_12k_MASTER[:,10][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Cuv   : [ ' + str(format(MVN_12k_MASTER[:,11][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Dubar   : [ ' + str(format(MVN_12k_MASTER[:,12][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('  Euv   : [ ' + str(format(MVN_12k_MASTER[:,13][sample_ind], '.6f')) + ', 0. ]\n')
        second.write('\n')
...
chi2_array_ALL_DATA_12k = np.array(chi2_vals)
np.save('chi2_array_ALL_DATA_12k.npy', chi2_array_ALL_DATA_12k)
print(chi2_vals)
#print(s)
#print(matches)
# for match in matches:

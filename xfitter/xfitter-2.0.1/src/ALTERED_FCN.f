...
c open(929, file='pars_saved.txt')
c do ik=1, MNE
c       write(929, *) parminuitsave(i)
c enddo
c close(929)
...
*     ---------------------------------------------------------
      ...
      call PDF_Param_Iteration(parminuit,iflag)
            open(919, file='chi2_values_.txt')
      write(919,*) chi2data_theory(iflag)
      close(919)
c the chi2out, or "f", should be the last thing returned by fcn
! [--- WS 2015-10-10
#ifdef TRACE_CHISQ
...
C Return the chi2 value:
      ...
      open(909, file='chi2_values.txt')
      do i=1,MNE
c C         parminuitsave(i) = parminuit(i)
      write(*,*) '\n the ith parameter is =', parminuitsave(i)
c c         write(*,*) 'the chi2 value is = \n', chi2out
        write(909,910) parminuitsave(i)
910   format(D8.3)

      enddo
            write(*,*) 'THE CHI2 VALUE IS'
      write(909,*) chi2data_theory
      close(909)

...
c       chi2_ = chi2data_theory(0)
c       write(929,669) chi2_
c 669   format(' ', D9.2)     
c       close(929)          
...
* Check if minuit converged OK
      ...
      do i=1,MNE
c        open(909, file='chi2_values.txt')
c C         parminuitsave(i) = parminuit(i)
c        write(*,*) 'the ith parameter is =', parminuitsave(i), '\n'
c c         write(*,*) 'the chi2 value is = \n', chi2out
c        write(909,910) parminuitsave(i)
c 910   format(D8.3)
c c         write(90,*) chi2out
       enddo
c       close(909)
      open(929, file='pars_saved.txt')
      do ik=1, MNE
            write(929, *) parminuitsave(i)
      enddo
      close(929)
